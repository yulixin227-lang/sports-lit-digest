...
class WxPusherTests(unittest.TestCase):
    def test_build_message_uses_public_digest_base_url(self):
        paper = {
            ...
            "ppt_ready_fields": {
                "needs_confirmation": "需阅读全文确认：全文 PDF、原文 Figure、具体效应量。若要做 PPT，必须使用原文 Figure，不可编造图片。",
            },
        }
        ...
        self.assertIn("【可讲点】", message["content"])
        self.assertIn("适合讲肥胖、骨关节炎和多模式康复管理的证据地图", message["content"])
        self.assertIn("【需确认】", message["content"])
        self.assertIn("需阅读全文确认：全文 PDF、原文 Figure、具体效应量", message["content"])
        self.assertIn("不可编造图片", message["content"])
